...
def setup_observability(service_name: str):
    ...
    # Базовая настройка стандартного логгера для перехвата сообщений от библиотек
    logging.basicConfig(
        format="%(message)s",
        stream=sys.stdout,
        level=logging.INFO,
    ...
    # Endpoint берется из переменной окружения или используется дефолт (для docker)
